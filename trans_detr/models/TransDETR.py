...
class ClipMatcher(SetCriterion):
    ...
    def loss_rec(self, outputs, gt_instances: List[Instances], indices, num_boxes, log=False):
...
#                 rotate_per_img = gt_per_img.rotate
#                 print(gt_per_img.word[J[J != -1]].shape)
...
class RuntimeTrackerBase(object):
    def __init__(self, score_thresh=0.65, filter_score_thresh=0.6, miss_tolerance=3):
        #dataset score_thresh, filter_score_thresh
        # ICDAR15 0.5 0.3  (COCOText: )
        # YVT 0.3 0.2
        # Minetto 0.4 0.3
...
#                 for a, b in zip(outputs_class[:-1], outputs_coord[:-1])]
    ...
    def _forward_single_image(self, samples, track_instances: Instances, taget = None, h = None ,w= None, time_cost=None):
...
#         out = {'pred_logits': outputs_class[-1], 'pred_rotate': outputs_rotate[-1], 'pred_boxes': outputs_coord[-1], 'ref_pts': ref_pts_all[5]}
        ...
        if self.training:
            # rotated roi
            rois =  torch.full((taget.boxes.shape[0], 6), 0.0,
                                    dtype=torch.float, device=rec_feature.device)
            cwh = taget.boxes * torch.tensor([w, h, w, h], dtype=torch.float, device=rec_feature.device)
...
#             recog_indx = track_instances.scores>0.5
            ...
            if track_instances.pred_boxes.shape[0] != 0:  
                rois =  torch.full((track_instances.pred_boxes.shape[0], 6), 0.0,
                                        dtype=torch.float, device=rec_feature.device)
                cwh = track_instances.pred_boxes * torch.tensor([w, h, w, h], dtype=torch.float, device=rec_feature.device)
        ...
        if not self.training and time_cost!=None:
            torch.cuda.synchronize()
            time_cost.update(dict(
                memory_embed_time=time.time() - start
            ))
            start = time.time()
    ...
    def inference_single_image(self, img, ori_img_size, track_instances=None):
        ...
        time_cost = {}
        res,time_cost = self._forward_single_image(img,
                      track_instances=track_instances, h = ori_img_size[0], w = ori_img_size[1], time_cost = time_cost)
    ...
    def forward(self, data: dict):
        ...
        for frame,taget in zip(frames,gt_instances):
            ...
            frame_res,time_cost = self._forward_single_image(frame, track_instances, taget,h,w)
...
def build(args):
    ...
    if args.memory_bank_type is not None and len(args.memory_bank_type) > 0:
        ...
        memory_bank = None
    losses = ['labels', 'boxes', 'rotate', 'rec']
