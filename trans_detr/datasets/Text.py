...
#         self.img_files = self.img_files[:int(len(self.img_files)/3)]
...
#                         continue
                    ...
                    labels0.append(list(map(float, text[:7])))
...
#         print(self.num_frames_per_batch)
    ...
    def __getitem__(self, idx):
        ...
        if self.vis:
            ...
            for img,ann in zip(images,targets):
                img = img.permute(1,2,0)
                print(img.shape)
               
                img = (img.cpu().numpy()*[0.229, 0.224, 0.225]+[0.485, 0.456, 0.406])*255
                h, w = img.shape[:2]
                label = ann["labels"]
                area = ann["area"]
                iscrowd = ann["iscrowd"]
                rotates = ann["rotate"]
                words = ann["word"]
                texts_ignored = ann["texts_ignored"]
                boxes = ann["boxes"] * torch.tensor([w, h, w, h], dtype=torch.float32)
                image = img.copy()
                boxes = box_cxcywh_to_xyxy(boxes)
                for box,rotate,word in zip(boxes,rotates,words):
...
#                     print(word)
#                     image=cv2AddChineseText(image,word.cpu().numpy(), (int(res[0,0]), int(res[1,0]) - text_size),((0,0,255)), text_size)

                cv2.imwrite("./exps/show/{}.jpg".format(random.randint(1,100)),image)
...
def build_dataset2transform(args, image_set):
    ...
    dataset2transform_train = {'ICDAR2015': mot17_train, 'YVT': mot17_train, 'BOVText': mot17_train,
